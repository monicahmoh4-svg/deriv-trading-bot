...
export default function Home() {
  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      const v1Token = params.get('token1');
      if (v1Token) {
        await handleV1Callback(params);
        return;
      }
      ...
      if (hash && hash.includes('token1=')) {
        ...
        if (hashToken) {
          await handleV1Callback(hashParams);
          return;
        }
      }
      ...
      if (code && state) {
        await handleV2Callback(code, state);
        return;
      }
    ...
    const handleV1Callback = async (p: URLSearchParams) => {
      const token = p.get('token1');
      if (!token) {
        router.replace('/login');
        return;
      }
      ...
      try {
        ...
        try {
          authResponse = (await ws.authenticate(decodeURIComponent(token))) as Record<string, unknown>;
        } catch {
        ...
        let finalBalance = 0;
        let finalCurrency = p.get('cur1') || 'USD';
    ...
    const handleV2Callback = async (code: string, state: string) => {
      ...
      try {
        ...
        console.error('V2 OAuth callback error:', err);
        setStatus(err instanceof Error ? err.message : 'Login failed');
        cleanupUrl(window.location.origin);
        setTimeout(() => router.replace('/login'), 3000);
      }
